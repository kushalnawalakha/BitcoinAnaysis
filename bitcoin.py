...
def computeDelta(wt, X, Xi):
    ...
    """
    # YOUR CODE GOES HERE
    sdx=np.std(X.values[0:-1])
    meanx=np.mean(X.values[0:-1])
    summation=0
    similiarityMeasureYi=[]
    xcomputed=X[0:-1]-meanx

    for xi in Xi.values:
        stdMul=len(X[0:-1])*sdx*np.std(xi[0:-1])
        temp=np.exp(wt*np.sum((xi[0:-1]-np.mean(xi[0:-1]))*xcomputed)/stdMul)
        summation=summation+temp
        similiarityMeasureYi.append(temp*xi[-1])
    returnValue =np.sum(similiarityMeasureYi)/summation;
    return returnValue
